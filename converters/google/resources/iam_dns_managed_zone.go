...
func DNSManagedZoneIamUpdaterProducer(d TerraformResourceData, config *Config) (ResourceIamUpdater, error) {
	...
	m, err := getImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/managedZones/(?P<name>[^/]+)", "(?P<project>[^/]+)/(?P<name>[^/]+)", "(?P<name>[^/]+)"}, d, config, d.Get("name").(string))
	if err != nil {
		return nil, err
	}
...
func DNSManagedZoneIdParseFunc(d *schema.ResourceData, config *Config) error {
	...
	m, err := getImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/managedZones/(?P<name>[^/]+)", "(?P<project>[^/]+)/(?P<name>[^/]+)", "(?P<name>[^/]+)"}, d, config, d.Id())
	if err != nil {
		return err
	}
...
func (u *DNSManagedZoneIamUpdater) qualifyManagedZoneUrl(methodIdentifier string) (string, error) {
	urlTemplate := fmt.Sprintf("{{DNSBasePath}}%s:%s", fmt.Sprintf("projects/%s/managedZones/%s", u.project, u.name), methodIdentifier)
...
func (u *DNSManagedZoneIamUpdater) GetResourceId() string {
	return fmt.Sprintf("projects/%s/managedZones/%s", u.project, u.name)
}
